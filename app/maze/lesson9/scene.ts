...
export class MazeScene extends Phaser.Scene {
  ...
  preload() {
    this.load.spritesheet('avatar', 'https://labs.phaser.io/assets/sprites/dude.png', {
      frameWidth: 32,
      frameHeight: 48,
    });
  ...
  create() {
    ...
    this.cameras.main.setBackgroundColor('#795548');
    ...
    this.player = this.add
      .sprite(this.gridX * this.tileSize, this.gridY * this.tileSize, 'avatar', 4)
      .setOrigin(0)
      .setDisplaySize(this.tileSize, this.tileSize);
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (
        this.maze[ey][ex] === 0 &&
        !(ex === 1 && ey === 1) &&
        !(ex === this.goalX && ey === this.goalY)
      ) {
        const enemy = this.enemies
          .create(ex * this.tileSize, ey * this.tileSize, 'enemy')
          .setOrigin(0)
          .setDisplaySize(this.tileSize, this.tileSize) as Phaser.GameObjects.Sprite & {
            gridX?: number;
            gridY?: number;
          };
        enemy.gridX = ex;
    ...
    this.physics.add.overlap(this.bullets, this.enemies, (bulletObj, enemyObj) => {
      ...
      const hitFx = this.add
        .text(enemyGO.x + enemyGO.displayWidth / 2, enemyGO.y + enemyGO.displayHeight / 2, 'ðŸ’¥', {
          fontSize: '26px',
        })
        .setOrigin(0.5)
        .setDepth(1000);
